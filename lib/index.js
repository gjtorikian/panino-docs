...
var globalObjs;
var globalObjsJSON = JSON.parse(fs.readFileSync(__dirname + "/objs.json", "utf8"));
...
function Panino(files, options) {
    ...
    try {
        if (typeof options.parseOptions === 'string') // it came from the command line
            options.parseOptions = JSON.parse(options.parseOptions);
    }
    catch (e) {
        console.error("Something went wrong trying to parse your JSON parse options:");
        console.error(e);
        process.exit(1);
    }
    ...
    if (options.parseOptions !== undefined) {
        ...
        parser.yy.useComma = options.parseOptions.useComma;
        parser.yy.useArrow = options.parseOptions.useArrow;
    }
        ...
        try {
            ...
            for (id in panini) {
                if (panini.hasOwnProperty(id)) {
                    ...
                    d.filenamePrefix =  Path.basename(file, Path.extname(file)); // this is stupid
 ...
 **/
Panino.prototype.toHTML = function (options, fullTree) {
    ...
    var _fullTree = JSON.parse(fullTree).tree.children;
    ...
    function markdown(text, inline) {
        ...
        r = namp.toHTML(text, {highlight: false } ).html;
        ...
        r = r.replace(/\[\[([\s\S]+?)\]\]/g, function (all, def) {
            ...
            if (!list[id]) { // it's in a different file, list refers to current page
                for (var o in globalObjs) {
                    var gObj = globalObjs[o];
                    if (id == gObj) {
                        var r = '<a href="' + id + '.html">' + id + '</a>';
                        return convertGlobalLink(r);
                    }
                }

                // search in the tree
                var found = false;
                for (var t in _fullTree) {
                    for (var c in _fullTree[t].children) {
                        var cObj = _fullTree[t].children[c];
                        if (cObj.id == id || found) {
                            obj = cObj;
                            found = true;
                            break;
                    ...
                    if (found) { // break out of inner loop too
                        break;
                    }
                }

                if (!found) {
                    // traverse is slow, which is why we don't always use it, but here we need to go 
                    // two levels deep; we may need to go more, and I don't want to write a bunch of
                    //  for...in loops for the children
                    var traverse = require('traverse');

                    try {
                        traverse(_fullTree).forEach(function(node) {
                                if (node.id == id) {
                                    obj = node;
                                    throw new Error("StopIteration"); // so sue me.
                                }
                        });
                    } catch(error) { if(error.message != "StopIteration") throw error; }

                    if (obj === undefined) {
                        console.error("ERROR: While replacing [[ ]], couldn't find " + id);
                    }
                }
            }
            else {
            ...
            var className = getClassName(obj.id, obj.name);

            var basePortion = obj.filenamePrefix + ".html#" + obj.id;

            var r = '<a href="' + basePortion + '" class="link-short" title="' + obj.id + (obj.type ? ' (' + obj.type + ')' : '') + '" data-id="' + obj.id + '">';
            r += def.join(' ') || obj.id;
            r += '</a>';
            return r;
        });
    ...
    function link(obj, short, classes) {
        ...
        if (!obj.path && !obj.id) { // it's not in the tree
            var found = false;
            for (var o in globalObjs) {
                if (globalObjs[o] == obj.id) {
                    found = true;
                    break;
                }
            }
            ...
            if (!found) {
                console.error("ERROR: Broken link for " + util.inspect(obj));
                process.exit(1);
            }
        }
        else {
            obj.path = obj.id; // useful for children, i.e. A.B
        }

        var basePortion = getAbsPath(obj.path);

        var r = '<a href="' + basePortion + '" class="' + (classes || []).join(' ') + '" title="' + obj.id + (obj.type ? ' (' + obj.type + ')' : '') + '" data-id="' + obj.id + '">';
    ...
    function internalLink(obj, member, short, classes) {
        var r = link(member, short, classes);
        var re = new RegExp(/href=\"(.+(?=.))html\"/ig);
        var m;
        ...
        if ( (m = r.match(re) ) ) {
            ...
            // we're dealing with member links, not objects, so things are a little different
            basePortion = getAbsPath(obj.path || obj, member.id);

            return r.replace(re, 'href="' + basePortion + '"');
        }
    ...
    function signature(obj, sig, classes) {
        ...
        if (obj.id.indexOf("new") == 0) {
            ...
            r = '<span class="eventObjName">' + className + '</span><span class="eventListenerStart"></span>';
            ...
            sig.args.forEach(function (sigArg, sigIdx, sigArgs) {
                ...
                if (obj.arguments && obj.arguments[sigIdx]) {
                    ...
                    currArg.types.forEach(function (currArgType, currIdx, currArgs) {
                        link += makeAPIObjLink(obj, currArgType, 'argument ' + (classes || []).join(' '));
    ...
    function argumentTable(args, tableClasses, trClasses, tdClasses) {
        var r = '<table class="' + (tableClasses || []).join(' ') + '">';
        ...
        for (var a in args) {
            ...
            if (args[a].optional === true) requiredText = "Optional";
            r += '<td class="argRequired ' + (tdClasses || []).join(' ') + '">' + requiredText + '</td>';

            r += '<td class="argDescription ' + (tdClasses || []).join(' ') + '">' + markdown(args[a].description) + '</td>';
            r += '</tr>';
        }
    ...
    function returnLink(obj, ret, classes) {
        var non_link = (obj.type == 'constant' || ret.type == 'Void' || ret.type == 'null' || ret.type == 'undefined' || ret.type === undefined);
        var text = "";
        var isGlobal = false;
        ...
        if (non_link)
            ...
            text = makeAPIObjLink(obj, ret.type, 'returnType ' + (classes || []).join(' '));
    ...
    // if this has a superclass, return the proper url;
    // else, just return yourself
    function getAbsPath(name, swapID) {
        var obj = list[name];
        var basePortion = "";

        if (obj === undefined) { // probably something like String, Object, e.t.c.
            basePortion = name + ".html";
        }
        else if (options.keepChildClasses === true && obj.superclass !== undefined) {
            if (swapID)
                basePortion = obj.superclass + ".html#" + swapID;
            else
                basePortion = obj.superclass + ".html#" + obj.id;
        }
        else {
            basePortion = obj.id + ".html";
            if (swapID)
                basePortion += "#" + swapID;
        }
    ...
    function makeAPIObjLink(obj, type, classes) {
            for (var o in globalObjs) {
                var gObj = globalObjs[o];
                if (type == gObj) {
                    var r = '<a href="' + type + '.html" class="' + classes + '" title="' + obj.id + (obj.type ? ' (' + obj.type + ')' : '') + '>' + type + '</a>';
                    return convertGlobalLink(r);
                }
            }
            var linkText = type;
                var traverse = require('traverse'); // TODO: so slow. must be a better way
                var obj;
                ...
                if (obj === undefined) {
                    console.error("ERROR: While creating a signature, couldn't find " + type);
                }

                var basePortion;
                if (obj.superclass === undefined) {
                    basePortion = type + ".html";
                }
                else {
                    basePortion = obj.superclass + ".html#" + type;
                }

                linkText = linkText || type[0]; // 0 for 'array' returns
                return '<a href="' + basePortion + '" class="' + classes + '" title="' + obj.id + (obj.type ? ' (' + obj.type + ')' : '') + '">' + linkText + '</a>';
    }

    function getClassName(id, memberName) {
        var pos = id.indexOf(memberName);

        if (pos < 0)
            return id;

        return id.substring(0, pos - 1); // remove ., #, @, e.t.c.
    }

    function convertGlobalLink(l) {
        for (var o in globalObjs) {
