...
function render_html(panino, options, callback) {
  ...
  md_conrefs.init(panino.files, {type: ".markdown"});
  ...
  function link(text, classes, short) {
      ...
      if (obj.file === undefined) {
          if (additionalObjsJSON !== undefined) {
              ...
              obj = {file: url, id: text, isAdditional: true};
          }
      }
  ...
  var assetsDirName = path.basename(options.assets);
  var outAssetsDirName = path.join(options.output, assetsDirName);
  ...
  if (options.split) {
    ...
    for (var key in list) {
      if (list.hasOwnProperty(key) && list[key].type === "class" && list[key].superclass === undefined) {
        ...
        // render HTML
        html = fn(vars);
        fs.writeFile(path.join(options.output, outFile + '.html'), html, callback);
      }
    }
  }
  else {
