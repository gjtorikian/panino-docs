 ...
 **/
Panino.parse = function parse(files, options, callback) {
  // TODO: Default args go here for some ungodly reason

  options.split = options.split || true;
  options.disableTests = options.disableTests || false;
...
/**
 *  Panino.render(name, ast, options, callback) -> Void
 ...
 **/
Panino.render = function render(name, ast, options, callback) {
  if (!renderers[name]) {
...
Panino.setParsingRules = function(options, parser) {
  ...
  if (options.parseOptions !== undefined) {
      ...
      parseOptionsJSON = JSON.parse(fs.readFileSync(__dirname + "/panino/defaultParseRules.json"));
  }
