...
var opts = nomnom
  ...
  .option('docPath', {
        ...
        metavar: 'STRING',
        default: 'http://www.nodemanual.org/latest/js_doc/%s.html',
  })
  .option('split', {
        abbr: 's',
        full: 'split',
  ...
  .option('parseOptions', {
        ...
        metavar: 'STRING',
        default: '{ "useDash": true}',
      })
  .option('skin', {
    help: 'Custom templates [' + __dirname + '/../skins/goose' + ']',
    default: path.join(__dirname, '..', 'skins', 'goose'),
    metavar: 'PATH',
  })
...
walk_many(opts.path, '\\.' + opts.extension + '$', function (filename, stat, cb) {
  ...
  // build trees
      if (opts.format == 'html' && opts.split == 'yes') {
        ...
        , format: opts.format, extension: opts.extension, parseOptions: opts.parseOptions, globalObjType: opts.globalObjType, docPath: opts.docPath, srcPath: opts.path}))
        };
     }
        else {
  ...
  switch (opts.format) {
    ...
    case 'html':
      wrench.copyDirSyncRecursive(path.join(opts.skin, 'skeleton'), parentOutDir + "/resources", {preserve: true});
      ...
      if (opts.split == 'yes')
      {
        paninoArray.forEach(function (n) {
          ...
          if (html[1] !== undefined)
            fs.writeFileSync(path.join(output, html[1].substring(0, html[1].lastIndexOf('.')) + '.html'), html[0]);
        });
      }
      else
      {
      ...
      funcDocs.runTests([output], {stopOnFail: false, ext: ".html"}, function(err) {
          if (err)
            console.error(err);
      });
      break;
