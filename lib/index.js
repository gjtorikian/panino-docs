...
function Panino(files, options) {
    ...
    try {
        ...
        console.error("Something went wrong trying to parse your JSON options:");
        console.error(e);
        process.exit(1);
    }
    ...
    if (options.parseOptions !== undefined) {
        ...
        parser.yy.useParentheses = options.parseOptions.useParentheses;
        parser.yy.useCurlies = options.parseOptions.useCurlies;
    }
        ...
        try {
            ...
            for (id in panini) {
                if (panini.hasOwnProperty(id)) {
                    ...
                    d.resultingFile =  Path.basename(file, Path.extname(file)) + ".html#" + d.id; // this is stupid
 ...
 **/
Panino.prototype.toHTML = function (options, fullList) {

    var Jade = require('jade'),
        //namp = require('namp'), // my own parser is broken atm
    ...
    // render link
    // obj can be a reference to an id--or for globals, a string itself
    function link(obj, classes, short) {
        if (typeof obj === 'string' && obj.indexOf("new") == 0) {
            obj = obj.substring(obj.indexOf(" ") + 1) + ".new";
        }

        var obj = fullList[obj.id] || fullList[obj] || obj;

        if (!obj.resultingFile) { // might be global
            for (var o in globalObjs) {
                if (obj == globalObjs[o]) {
                    obj = {};
                    obj.resultingFile = options.docPath.replace("%s", globalObjs[o]);
                    obj.id = globalObjs[o];
                    break;
                }
                else if (obj.id == globalObjs[o]) { // for short links
                    obj.resultingFile = options.docPath.replace("%s", globalObjs[o]);
                    break;
                }
            }
            ...
            if (!obj.resultingFile) {
                console.error("While trying to make a link, I do not know what this is: " + util.inspect(obj));
                console.trace();
                process.exit(1);
            }
        }
            
        var r = '<a href="' + obj.resultingFile +
                '" class="' + classes +
    ...
    function internalLink(obj, classes, short) {
        var r = link(obj.id, classes, short);
