...
function Panino(files, options) {
    ...
    files.forEach(function (file) {
        ...
        try {
            ...
            if (options.parseOptions !== undefined) {
                ...
                parseOptionsJSON = { }; // default to defaults
    ...
    for (i = t.length - 1; i >= 1; i -= 1) {
        ...
        if (idx >= 0) {
            ...
            if (p) {
                // leave subclasses alone; probably modify for opts.split ?
                if (tree[id].type !== "class") {
    ...
    for (id in list) {
        if (list.hasOwnProperty(id)) {
            ...
            // sometimes, memebrs can be defined across other files
...
function convertMD(file, text) {
    ...
    if (re = textLines[0].match(/^\#{1}\s+(.+)/)) { // if #, it's a title
    ...
    else if (re = textLines[0].match(/^\#{2}\s+(.+)/)) { // if ##, it's a class
        if (!firstClass.found) {
            firstClass.found = true;
            firstClass.name = re[1];
            textLines[0] = "/**\n* " + "class " + firstClass.name;
            if ( (metaString = textLines[1].match(metaExpr) ) ) {
                var metaString = metaString[0].replace("(", "").replace(")", "");
                textLines[0] = "/** " + metaString + "\n* " + "class " + firstClass.name;
                textLines[1] = "* ";
            }
            else
                textLines[1] = "* ";
    ...
    for (var l = 2; l < textLines.length; l++) {
        if (re = textLines[l].match(/^\#{2}\s+(.+)/)) { // if ##, it's a class
            if (!firstClass.found) {
                firstClass.found = true;
                firstClass.name = re[1];
        ...
        else if (textLines[l].match(/^\#{3}\s+/)) { // if ###, it's a member
           ...
           // while (textLines[n].match(/[-|*|+]\s*.+?\s*{.+?}\s*/)) {
            ...
            var tagRE = textLines[l].match(/^(\((.+)\))$/);
            
            if (tagRE) { // if it's got tags, add them appropriately to the starting /**
                textLines[startingDefPos] = textLines[startingDefPos].replace("/**", "/** " + tagRE[2]);
                textLines[l] = "";
            }
...
Panino.prototype.toHTML = function (options, fullList, outFile) {
    ...
    function link(text, classes, short) {
        ...
        // TODO: somehow, sometimes, nonce can get through
        // perhaps an error in source file ?
        if (short && obj.name && obj.name.match(/@-=@=-@(\d+)@-=@=-@/)) {
            if (obj.id.indexOf("@") >= 0)
                obj.name = obj.id.substring(obj.id.indexOf("@") + 1);
            else
                obj.name = obj.id.substring(obj.id.lastIndexOf(".") + 1);
        }
