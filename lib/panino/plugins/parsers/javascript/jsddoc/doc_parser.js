...
function parse_loop(input) {
    ...
    while(!input.eos()) {
      if (look(/@class\b/))
        ...
        match(/@/);
        var tagName = look(/\w+/);
        tag = meta_tags[tagName];
        if (tag)
          meta_at_tag(tag, tagName);
        else {
...
function meta_at_tag(tag, tagName) {
  var prev_tag = current_tag;

  add_tag("meta");
  current_tag["name"] = tag.key;
  match(/\w+/); // consumes meta tag name
  skip_horiz_white();
  ...
  else { // For singleline tags, scan to the end of line and finish the tag
...
// matches @link [<ident-chain> some text...]
// sugar this into panino style [[ ]] link block
function at_link() {
  match(/\{@link/)
  skip_horiz_white();

  var linkText = strip(match(/.*?\}/)); 

  // TODO: figure out why regexp above doesn't match \{
  current_tag["doc"] = current_tag["doc"].slice(0,-1) + 
                       "[[" + 
                       linkText.replace(/\s*@link\s*/, "").slice(0,-1)
                       + "]] ";
