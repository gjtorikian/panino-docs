...
panino.main = exports.main = function (args, callback) {
    ...
    default:
    	...
    	abbr: 'u',
        help: 'Resulting file(s) location for the assets [./out/resources]',
        metavar: 'PATH',
    default:
        './out/resources',
    }).option('format', {
    ...
    default:
        ...
        abbr: 'd',
        full: 'disableTests',
        help: 'Disables the test suite that runs at the end of an HTML build.',
        flag: true,
    }).option('parseOptions', {
        abbr: 'p',
        full: 'parse-options',
    ...
    var manifest = {};
    try {
        // not using require for node < v0.4 compatibility
    ...
    //
    var files = [];
    md_conrefs.init(opts.path, {type: opts.extension}); // collect conref IDs
    walk_many(opts.path, '\\.' + opts.extension + '$', function (filename, stat, cb) {
        ...
        if (opts.format == 'html' && opts.split) {
            for (var f = 0; f < safeFiles.length; f++) {
                ...
                paninoArray.push(new Panino(f_a, {
                    ...
                    srcPath: opts.path,
                    split: true,
                    silent: true // supress "building" message; see below TODO for more
                }))
            };
        ...
        wrench.mkdirSyncRecursive(opts.outputAssets, "0755");

        console.log("Converting to " + opts.format + "...");
        switch (opts.format) {
        case 'ast':
            fs.writeFileSync(output + "/" + path.basename(output).replace("/", "").replace("\\", "") + ".json", panino.toAST(opts));
            callback();
            break;

        case 'js':
            fs.writeFileSync(output + "/" + path.basename(output).replace("/", "").replace("\\", "") + ".js", 'var panino = ' + panino.toAST(opts) + ';');
            callback();
            break;
        ...
        case 'html':
            ...
            wrench.copyDirSyncRecursive(path.join(opts.skin, 'skeleton'), opts.outputAssets, {
                preserve: true
            });
    ...
    // put them in the right place in the split tree
                ...
                if (d.extension === true) {  
                    ...
                    for (var a in paninoArray) {
                        ...
                        if (srcObj !== undefined) { // found the parent
                            ...
                            var found = false;
                            // sorted push to get member in the right place
