...
function build_tree(files, nodes, options) {
  ...
  _.each(nodes, function (node, id) {
    function convertToHierarchy(inheritArray)
    {
        // Build the node structure
        var rootNode = [];
        _.each(inheritArray, function(path) {
            rootNode.push(buildNodeRecursive(rootNode, path, 0));
        });
    ...
    function buildNodeRecursive(node, path, idx)
    {
        if (nodes[path] !== undefined && nodes[path].inherits) {
          var h = {};
          h.id = path;
          h.children = nodes[path].children;
    ...
    if ('class' === node.type) {
      ...
      if (node.inherits) {
        node.hierarchy = convertToHierarchy(_.uniq(node.inherits));
      }
...
process.exit(1)
  ...
  // but we want these apf.* items into their own segments
  ...
  // insert inherits information; does it matter that it's down here?
  _.each(children, function(c) {
      var inheritedClasses = {};
          
      
      if (c.inherits) {
          //inheritedChildrenMap(c.inherits);
          //c.inherited = inheritedClasses;
  
          //console.log(util.inspect(c.inherits, null, 15));
      }
  });
