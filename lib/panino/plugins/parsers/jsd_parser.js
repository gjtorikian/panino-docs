...
var process_jsd = function(source, file, options, callback) {
  var nodes = {}, remainingNodes, ast, docs, expanded, merged, classPrefix, className, memberName, list = {}, functions = [], extractedFunctions = [], tree, parted, sections, children;
  doc_ast = new DocAst();
  ...
  if (options.splitByClass) {
    while (merged.length > 0) {
      ...
      if (options.report) {
        ...
        else { // basically, we're at the last class, so just scoop everything in
        ...
        if (availableFunctions.length > 0) {
          _.forEach(availableFunctions, function(func) {
            ...
            if (func.name.charAt(0) !== "$")
              reportObject[className].push({name: func.name, reason: "Method isn't documented at all"});
          });
        }

        //console.log(util.inspect(reportObject, null, 5));
        //process.exit(1)
      }
    }
  }
  ...
  function formatNodes(merged) {
    ...
    remainingNodes = _.reject(remainingNodes, function(i) {
      ...
      if (i["tagname"] === "method" || i["tagname"] === "constructor" || isEvent) {
        ...
        if (options.report && i["tagname"] === "method") {
              ...
              // now make sure the parameters match, too
                ...
                _.any(func.params, function (funcArg, idx) {
                  ...
                  if (idx >= nodes[memberName]["arguments"].length) {
                    reportObject[className].push({name: nodes[memberName], reason: "Missing documented arguments"});
                    return true;
                  }
                  // catch instance where arg and func names don't match
                  else if (funcArg.name !== nodes[memberName]["arguments"][idx].name) {
                    reportObject[className].push({name: nodes[memberName], reason: "Documented arguments missing proper name"});
                    return true;
                  }
                });
              }
