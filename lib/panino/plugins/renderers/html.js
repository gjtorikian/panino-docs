...
function render_html(panino, options, callback) {
  ...
  // reset if conrefs exist; useful if calling the Panino render multiple times
  if (_.size(md_conrefs.getData()) == 0) {
  ...
  if (options.split) {
    ...
    for (var key in list) {
      if (list.hasOwnProperty(key) && list[key].type === "class" && list[key].superclass === undefined) {
        ...
        if (options.suffix)
          outFile += options.suffix;
        if (options.prefix)
          outFile = options.prefix + outFile;
