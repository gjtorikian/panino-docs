...
function render_html(panino, options, callback) {
  ...
  function link(text, classes, short) {
    ...
    if (typeof text === 'object') {
      ...
      if (obj.file === undefined) {
          if (additionalObjsJSON !== undefined) {
              ...
              if (url === undefined) {
                  ...
                  if (url === undefined) {
                      console.error("Error".red + ": " + util.inspect(text) + " has no valid link! (classes: " + classes + ")");
                      console.trace();
                  }
              }
      ...
      if (options.split) {
        ...
        if (!obj.isAdditional) {
          ...
          if ( (m = linkFile.match(hrefRE) ) ) {
            ...
            linkFile = linkFile + ".html#" + obj.path;
          }
        }
      }
      else {
     ...
     if (options.linkFormat) {
         linkHtml = options.linkFormat(linkHtml);
     }
     
      var r = '<a href="' + linkHtml.href +
              '" class="' + linkHtml.class +
    ...
    function returnLink(obj, ret, classes) {
        ...
        try {
          if (non_link)
            ...
            console.error("FATAL".red + ": someone set up us the bomb while trying to create a return link for ", util.inspect(obj, null, 5), ret);
            console.error(e);
            process.exit(1)
        }
  ...
  function markdown(text, inline) {
    ...
    r = r.replace(/\[\[([\s\S]+?)\]\]/g, function (all, def) {
      ...
      if (def.length == 0) {
        if (obj.type === undefined && additionalObjsJSON[obj.name] === undefined) {
        ...
        else if (obj.type !== "class" && obj.type.indexOf("property") < 0)
          obj.name = "<code>" + obj.name + "()</code>";
        else
          obj.name = "<code>" + obj.name + "</code>";
      }
