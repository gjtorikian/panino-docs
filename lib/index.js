...
Panino.prototype.toHTML = function (options) {
    ...
    */
    ...
    function internalLink(obj, short, classes) {
        var r = link(obj, short, classes);
        ...
        if ( (m = r.match(re) ) ) {
            ...
            var className = id[0].substring(6);

            if (options.keepChildClasses !== true)
                className += "." + id[1];

            r = r.replace(re, 'href="' + className + '.html#' + obj.id + '"');
        }
            ...
            sig.args.forEach(function (sigArg, sigIdx, sigArgs) {
                ...
                if (obj.arguments && obj.arguments[sigIdx]) {
                    ...
                    currArg.types.forEach(function (currArgType, currIdx, currArgs) {
                        link += '<a href="' + getAbsPath(currArgType) + '">' + currArgType + '</a>';
    ...
    function returnLink(obj, ret, classes) {
        ...
        if (non_link)
            ...
            var basePortion = getAbsPath(ret.type);
            linkText = linkText || ret.type[0]; // 0 for 'array' returns
            text = '<a href="' + basePortion + '" class="returnType ' + (classes || []).join(' ') + '" title="' + obj.id + (obj.type ? ' (' + obj.type + ')' : '') + '">' + linkText + '</a>';
